		...
		if ($(window).width() > 600){
			...
			$.getJSON(url, function(json){
				...
				for (var i=0; i<length; i++){
					...
					item.innerHTML = '<p>' + linkify(json[i].text) + '</p><small class="tweet-time">'+relativeDate(json[i].created_at)+'</small>';
					fragment.appendChild(item);
				}

				var play = function(){
					timeout = setTimeout(function(){
						feed.animate({top: '-='+50}, speed, function(){
